...
std::vector<uint8_t> encode_socks5_connect_request(uint8_t atyp,
                                                    const std::string& host,
                                                    uint16_t port) {
    std::vector<uint8_t> req = {0x05, 0x01, 0x00, atyp};
    if (atyp == 0x01) {
        in_addr ipv4{};
        if (::inet_pton(AF_INET, host.c_str(), &ipv4) != 1) return {};
        const auto* b = reinterpret_cast<const uint8_t*>(&ipv4);
        req.insert(req.end(), b, b + 4);
    } else if (atyp == 0x04) {
        in6_addr ipv6{};
        if (::inet_pton(AF_INET6, host.c_str(), &ipv6) != 1) return {};
        const auto* b = reinterpret_cast<const uint8_t*>(&ipv6);
...
bool create_nonblocking_tcp_socket(const std::string& host, uint16_t port,
                                   socket_t& sock, bool& connected,
                                   std::string& error) {
    sock = kInvalidSocket;
    connected = false;
    addrinfo hints{};
    hints.ai_socktype = SOCK_STREAM;
    // Prefer IPv4 first since many servers lack IPv6 connectivity
    hints.ai_family = AF_INET;  // IPv4 only for better compatibility
    ...
    if (gai_ret != 0) {
        ...
        addrinfo* result6 = nullptr;
        const int gai_ret6 = ::getaddrinfo(host.c_str(), port_text.c_str(), &hints, &result6);
        if (gai_ret6 != 0) {
#ifdef _WIN32
    ...
    for (addrinfo* ai = result; ai != nullptr; ai = ai->ai_next) {
        socket_t cand = static_cast<socket_t>(::socket(ai->ai_family,
                                                        ai->ai_socktype,
                                                        ai->ai_protocol));
        ...
        const int ret = ::connect(cand, ai->ai_addr,
                                  static_cast<int>(ai->ai_addrlen));
        if (ret == 0) {
            sock = cand; connected = true;
            freeaddrinfo(result);
            return true;
        }
    ...
    if (error.empty()) error = nb_error.empty() ? "connect failed" : nb_error;
    return false;
}
...
proxy::EventFlags interest(const Peer& peer) {
    ...
    // Open state: backpressure on downlink
    EventFlags flags = EventFlags::None;
    if (!peer.pending_downlink.above_high_water())
...
bool finish_nonblocking_connect(Peer& peer, bool& send_open_ok, std::string& error) {
    ...
    PROXY_LOG(Info, "[upstream] stream=" << peer.h2_stream_id << " starting SOCKS5 handshake");
    return process_write(peer);
}

bool process_write(Peer& peer) {
    if (peer.state == State::Connecting) {
        bool ok; std::string err;
        return finish_nonblocking_connect(peer, ok, err);
    }
    if (peer.state == State::ProxyMethodWrite ||
        peer.state == State::ProxyConnectWrite) {
        std::size_t sent = 0;
        while (peer.control_out_offset < peer.control_out.size() &&
               sent < kUpstreamWriteBudgetBytes) {
    ...
    std::size_t sent = 0;
    while (peer.pending_uplink_offset < peer.pending_uplink.size() &&
           sent < kUpstreamWriteBudgetBytes) {
        const auto* data = peer.pending_uplink.data() + peer.pending_uplink_offset;
        const std::size_t remaining =
            peer.pending_uplink.size() - peer.pending_uplink_offset;
#ifdef _WIN32
        const int ret = ::send(peer.sock,
                               reinterpret_cast<const char*>(data),
                               static_cast<int>(remaining), 0);
...
bool process_read(Peer& peer) {
    if (peer.state == State::Connecting) {
        bool ok; std::string err;
        return finish_nonblocking_connect(peer, ok, err);
    }
    if (peer.state != State::Open) {
        // SOCKS5 control read states
        std::array<uint8_t, 512> buf{};
        while (peer.control_in.size() < peer.control_expected) {
            const std::size_t want =
                std::min(buf.size(), peer.control_expected - peer.control_in.size());
#ifdef _WIN32
            ...
            peer.control_in.insert(peer.control_in.end(),
                                   buf.begin(), buf.begin() + ret);
        }
        if (peer.state == State::ProxyMethodRead) {
            if (peer.control_in.size() < 2 ||
                peer.control_in[0] != 0x05 ||
                peer.control_in[1] != 0x00) {
                PROXY_LOG(Error, "[upstream] stream=" << peer.h2_stream_id
                              << " SOCKS5 method rejected ver=" << (int)peer.control_in[0]
                              << " method=" << (int)peer.control_in[1]);
                return false;
            }
            PROXY_LOG(Info, "[upstream] stream=" << peer.h2_stream_id
                          << " SOCKS5 method accepted, sending CONNECT");
            peer.control_out = encode_socks5_connect_request(
                peer.requested_atyp, peer.requested_host, peer.requested_port);
            if (peer.control_out.empty()) {
                PROXY_LOG(Error, "[upstream] stream=" << peer.h2_stream_id
                              << " failed to encode CONNECT request");
                return false;
            }
            peer.control_out_offset = 0;
            peer.control_in.clear();
        ...
        if (peer.state == State::ProxyConnectReadHead) {
            if (peer.control_in.size() < 4 ||
                peer.control_in[0] != 0x05 ||
                peer.control_in[1] != 0x00) {
                PROXY_LOG(Error, "[upstream] stream=" << peer.h2_stream_id
                              << " SOCKS5 CONNECT failed ver=" << (int)peer.control_in[0]
                              << " rep=" << (int)peer.control_in[1]);
                return false;
            }
            const uint8_t atyp = peer.control_in[3];
            peer.control_in.clear();
            if      (atyp == 0x01) { peer.control_expected = 6;  peer.state = State::ProxyConnectReadBody; }
            else if (atyp == 0x04) { peer.control_expected = 18; peer.state = State::ProxyConnectReadBody; }
            else if (atyp == 0x03) { peer.control_expected = 1;  peer.state = State::ProxyConnectReadDomainLength; }
            else return false;
            return true;
        }
...
std::string describe_route(const ServerConfig& config,
    ...
    if (!config.has_fixed_target ||
        config.target_type == ServerConfig::TargetType::Direct)
    ...
    return config.fixed_host + ":" + std::to_string(config.fixed_port) +
           " (socks5 for " + requested_host + ":" + std::to_string(requested_port) + ")";
}
